...
3. agent-deploy는 버리지 않고 core로 확장한다.
   - 현재는 베타 배포 가능한 reference implementation이며, 운영 자동화는 계속 보강한다.
...
- installer / Markdown asset 배포 엔진: agent-deploy/
```
...
- [x] D05 지식 DB 위치 확정
  - 결정: Google Drive + GitHub (이번 버전에서 실시간 메신저/Notion 제외)
...
현재 `agent-deploy/`는 베타 배포 가능한 reference implementation까지 확장됐지만, 전사 운영 자동화와 승격 workflow는 계속 보강한다. 다음 순서로 확장한다.
...
7. [x] asset pack Phase 2 planner/apply 1차 통합
   - CLI `--pack DIR[,DIR]`
   - base bundle + pack manifest composition
...
11. [x] runtime conflict-resolution decision capture 구현
   - `--conflict-resolution <json-file>` 옵션 추가
   - decision record 최소 validator 추가
   - install-state `source.conflictResolutions[]` 기록
   - `add-namespaced` copy-file destination을 `shared/<pack-id>/...`로 변환
   - invalid decision 거부 / add-namespaced transform / apply provenance smoke test
```
...
- [x] `mcp-baseline`은 default 제외 또는 profile별 선택
  - minimal/core에서는 제외하고, developer/full 또는 명시 `--module mcp-baseline`에서만 설치하도록 정리
...
- [x] bundle build script 추가 (`scripts/build-bundle.js`, `npm run bundle`; 의존성 0 순수 Node ZIP writer)
- [x] zip naming 정책 결정 → OS 공통 단일 zip, 버전 포함 + 고정 alias
  - `company-agent-kit-<version>.zip` (정식) + `company-agent-kit.zip` (동일 바이트 alias), 각 `.sha256` 동반
  - OS별 zip은 기존 결정(OS 공통)대로 제외. install.sh/install.bat을 한 zip에 동봉
- [x] zip 내부 구조 확정 (실제 런타임 레이아웃 기준, `src/cli.js` 엔트리)
- [x] zip checksum 생성 (SHA-256, `sha256sum -c` 호환 sidecar; 빌드 재현성 보장)
- [x] release manifest 생성 (`release-manifest.json` + `release-manifest.json.sha256`; zip/sidecar size·SHA-256 기록)
- [x] checksum 검증 안내 작성 (README 메인테이너 절차 + SETUP_WIZARD 사용자 다운로드 검증)

확정 구조:

```text
company-agent-kit/            # zip 내부 최상위 폴더(버전 무관 고정)
  ├─ src/                     # CLI + 모듈 (엔트리: src/cli.js)
  ├─ scripts/                 # 런타임 도달(check-asset-schema/catalog-parity) + validate
  ├─ manifests/
  ├─ schemas/                 # 필수: install-state 런타임 검증이 로드
  ├─ docs/                    # operator 문서(ASSET_PACKS / ASSET_SCHEMA_AND_CATALOG / SHARED_FOLDER_GUIDE)
  ├─ assets/
  ├─ package.json             # 필수: "type":"module" + bin (없으면 node src/cli.js 실패)
  ├─ README.md
  ├─ SETUP_WIZARD.md
  ├─ INSTALL_GUIDE.html       # 비개발자/처음 사용자용 시각 설치 가이드(SVG 포함, 브라우저로 열기)
  ├─ install.sh               # exec bit 보존
  └─ install.bat
```
...
```text
planner.js → pack-composer → pack-validator 가 import 시점에 scripts/check-asset-schema.js,
check-catalog-parity.js 를 eager-load 한다. 따라서 package.json "files"에 scripts/ 가 빠져 있으면
배포본의 node src/cli.js 가 모듈 해석 실패로 죽는다. files 배열에 scripts 추가로 수정.
후속: src/ 가 scripts/ 를 참조하지 않도록 분리(공유 체커를 src로 이동)하는 리팩토링 별도 진행.
```
...
- [x] secret scan (`scripts/check-secret-scan.js`, validate에 통합 — 고신호 credential 패턴(private key/AWS/GitHub/Google/OpenAI/Anthropic/채팅 플랫폼 토큰), `${ENV}` placeholder·`secret-scan:allow` marker 허용)
...
npm run validate는 8종 가드(manifest/rule-drift/entry-parity/asset-schema/catalog-parity/unicode-safety/secret-scan/MCP-governance)를 실행하고, asset pack/update/repair/uninstall/bundle 보강까지 포함한 smoke test는 83개다.
...
- [x] MCP를 minimal/core profile에서 제외
- [x] MCP allowlist 도입 (`assets/mcp/allowlist.json`, `check-mcp-governance.js`)
- [x] filesystem MCP 기본 제외
- [x] npx 기반 MCP는 버전 고정 검증
- [x] `DISABLED_MCPS` 환경 필터 구현 및 skip reason 기록
- [x] MCP token/env 값은 `${ENV}` placeholder만 허용

### 7.3 artifact signing

- [x] Windows exe signing 제외 결정
- [x] zip checksum 생성 (`scripts/build-bundle.js`가 SHA-256 sidecar 생성, 5.2 참조)
- [x] release manifest 생성 (`scripts/build-bundle.js`가 `release-manifest.json` 생성, 5.2 참조)
- [x] 내부 배포 채널에서 checksum 검증 안내 (README/SETUP_WIZARD에 `sha256sum -c`, PowerShell `Get-FileHash` 예시)

---

## 8. governance 자동화

agent-deploy만으로 전부 커버하지 않는 영역이다. 별도 automation 또는 GitHub/Drive 연동이 필요하다.
...
- [x] OS 공통 zip bundle 준비 (build script/checksum smoke test 완료)
- [x] SETUP_WIZARD.md 준비
- [x] Codex/Claude/Gemini 중 최소 2개 target 안정화
- [x] project scope 설치 기본값 확인
...
- [x] knowledge sharing rule 배포
- [ ] 공용 AI 계정 담당자 지정
- [ ] Drive 운영 메모, 공유 폴더, feedback 폴더, governance 기록 위치 준비
- [ ] Prompt DB 준비
- [ ] 첫 weekly 회고 일정 예약
...
2. Prompt DB/GitHub governance 자동화
3. 전사 rollout
```
