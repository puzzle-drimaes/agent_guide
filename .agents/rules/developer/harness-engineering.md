...
Claude, Codex, Gemini, Cursor 등 서로 다른 AI 도구가 같은 회사 방향성과 기술 규칙으로 일하게 만드는 규칙이다.
...
| Capability | Claude | Codex | Gemini | 처리 방식 |
|---|---|---|---|---|
| root instruction | native | native | native | AGENTS/CLAUDE/GEMINI |
| rules file | native/fallback | native/fallback | fallback | include or copy |
| skill | native | native/fallback | fallback | skill or instruction |
| command | native | fallback | fallback | command or prompt |
| MCP | native | native | optional | allowlist |
| install-state | tool-owned | tool-owned | tool-owned | .agent-deploy |
