...
4. `.agents/externals/skills` 또는 `.agents/externals/prompts`가 비어 있으면
...
5. Drive에서 받을 때 agent는 `.md` 파일 직접 다운로드를 우선한다.
   - 직접 다운로드 후 보안 검사(추천): Drive의 `.md` 원본을 externals 검역 위치에 저장한 뒤 로컬 파일에서 credential/customer data/민감정보/출처 누락을 점검하고 필요한 경우 redaction/sanitized copy 생성
   - 원본 그대로 받기: Drive의 `.md` 원본을 externals에만 저장하고, 검증 전 공유본으로 표시
...
7. 사용자가 컨펌하면 내 프로젝트의 아래 위치에 넣는다.
     <repo>/.agents/externals/skills/    (스킬)
     <repo>/.agents/externals/prompts/   (프롬프트)
...
즉 1차 도입의 기본 흐름(사람이 내려받아 externals에 넣기)은 연동 없이도 동작한다. 연동은 편의 기능이다.
...
내려받아 `.agents/externals/`에 넣는 수동 fallback을 안내한다.
...
- 선택한 .md 파일을 원형 그대로 다운로드해서 `.agents/externals/`에 저장하고 로컬에서 보안 검사해줘.
```
...
이 폴더의 핵심 흐름은 "올린 .md를 내려받아 `.agents/externals/`에 넣기"다. 따라서 파일이 진짜 `.md`로 유지돼야 한다.

```text
- agent(MCP)가 받을 때도 본문/base64 수동 복사보다 `.md` 파일 직접 다운로드를 우선한다.
- 다운로드한 원본은 externals 검역 위치에 둔 뒤, 로컬에서 보안/출처 검사를 수행한다.
