...
- `core`: `minimal` + code-review agent + prompt/knowhow capture skills + prompt library + MCP baseline.
...
- `governance`: `minimal` + KPI/quarterly review/prompt DB curation + knowhow promotion skills.
...
| `knowhow` | 숙련자/팀의 재사용 가능한 작업 노하우 | 향후 `assets/knowhow/` 또는 외부 asset pack으로 추가 |
...
`knowhow`를 Markdown asset으로 정리해 공유·검증·승격할 수 있어야 합니다.
...
**공유 knowhow 추가** → 개인/팀 문서를 바로 target 파일에 복사하지 않고, 먼저 asset type,
owner, audience, stability, review status를 정한 뒤 module/profile에 연결합니다. Pilot 단계에서는
`prompts/` 또는 별도 외부 asset pack으로 시작하고, 운영 단계에서 `assets/knowhow/`와 catalog를
도입합니다.
...
- `prompts/templates/knowhow` frontmatter schema와 asset catalog를 blocking validation으로 승격.
- 외부 asset pack 적용(`--asset-root` 또는 `apply-pack`)과 개인/팀 knowhow 승격 workflow.
