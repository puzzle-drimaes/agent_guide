...
하나의 정규 소스(`assets/`)를 여러 AI 코딩 툴(Codex, Claude Code, Cursor, …)의
...
| 정규 소스 | Codex | Claude Code | Cursor |
|-----------|-------|-------------|--------|
| `assets/rules/**` | `.agents/rules/**` + `AGENTS.md` managed block | `.claude/rules/**` (구조 보존) | `.cursor/rules/<flat>.mdc` (평탄화+.mdc) |
| `assets/skills/<n>/SKILL.md` | `.agents/skills/<n>/SKILL.md` | `.claude/skills/<n>/SKILL.md` | `.cursor/skills/<n>/SKILL.md` |
| `assets/agents/<a>.md` | `.codex/agents/<a>.md` | `.claude/agents/<a>.md` | `.cursor/agents/<a>.md` |
| `assets/commands/plan.md` | **skip+reason** (Codex adapter엔 슬래시 명령 설치 표면 없음) | `.claude/commands/plan.md` | **skip+reason** (Cursor엔 슬래시 명령 없음 — install-state에 기록) |
| `assets/mcp/servers.json` | `.codex/config.toml` 에 add-only TOML merge | `.mcp.json`(project)/`~/.claude.json`(home) 에 deepMerge | `.cursor/mcp.json` 에 deepMerge |
