...
- Prefer the GitHub `prompts` branch for prompt candidates. During the current WIP period, `main` direct push is not blocked.
- If the reusable asset is a skill rather than a prompt, prefer the GitHub `skills` branch instead.
```
