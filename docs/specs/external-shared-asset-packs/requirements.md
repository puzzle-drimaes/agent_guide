# AI-Knowhow / Shared Asset Packs — Requirements
...
외부에서 가져와 현재 프로젝트에 적용하려는 모든 Markdown은 특정 `AI-Knowhow` 폴더에서 관리되어야 한다.
다른 구성원이 공유하고 싶은 skill 또는 문서도 먼저 이 폴더에 넣고, agent는 이를 candidate asset pack으로
해석해 검증/요약/적용 후보를 만든다. AI-Knowhow 파일은 기존 문서, canonical rule, entry file을 직접 덮어쓰면 안 된다.
...
| FR-11 | `<repo>/AI-Knowhow/`의 Markdown 파일을 candidate pack으로 해석할 수 있어야 한다. |
