...
| MCP config | Governance-filtered JSON merge: project `.mcp.json`, home `.claude.json` | Governance-filtered TOML merge: `.codex/config.toml` | Skip with reason | Governance-filtered JSON merge: `.cursor/mcp.json` | allowlist/`DISABLED_MCPS` 적용. Gemini MCP는 안정된 project-scope contract 전까지 의도적 skip. |
...
| core | Yes | 간접 | 간접 | 간접 | prompt-asset, prompt-library, MCP 기본 제외 |
| developer | Yes | Yes | Yes | Yes | developer rules/skills/agents, command skip reason, MCP governance/filter |
...
- MCP는 allowlist/`DISABLED_MCPS` 필터 후 JSON merge로 기존 사용자 key를 보존한다.
...
- MCP는 allowlist/`DISABLED_MCPS` 필터 후 `.cursor/mcp.json`에 merge한다.
...
MCP governance 1차 구현 상태는 다음과 같다.

```text
- minimal/core에는 MCP 기본 제외
- developer/full 또는 --module mcp-baseline에서만 명시 설치
- assets/mcp/allowlist.json 기반 allowlist 검증
- filesystem MCP 기본 제외(defaultExcludedServers)
- npx 기반 MCP는 package version pin 필수
- token/env 값은 ${ENV} placeholder만 허용
- DISABLED_MCPS=server-a,server-b 런타임 필터와 skip reason 기록
- Gemini는 stable project-scope MCP config contract 전까지 skip+reason 유지
```
