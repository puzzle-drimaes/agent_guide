...
- prompt/skill 후보는 `prompts`/`skills` branch에 남기는 것을 우선한다. 현재 WIP 기간에는 `main` 직접 push를 차단하지 않는다.
```
...
- 통과한 후보만 `main`에 반영한다. 안정화 후에는 protected merge로만 반영한다.
...
- 성공률 ≥ 80%
- 문구 안정(최근 수정 없이 재사용)
→ 모두 충족 시에만 사람 확인 후 main 반영 또는 승격
```
...
Prompt 후보 등록(Drive + GitHub prompts branch)
  ...
  → 월간 또는 분기 정리 때 검증
  → owner 지정
  → main 반영 또는 assets/skills/company-* 로 승격
  → installer 배포
```
...
| Reviewer | AI governance / 계정 담당자 | candidate→main 반영 검토, 정리(중복/deprecate), 승격 게이트 판정 |
...
candidate ──(운영자 수동 확인 + main 반영)──> main
