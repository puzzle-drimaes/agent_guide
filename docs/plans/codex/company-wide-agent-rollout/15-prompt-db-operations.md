...
- 성공률 ≥ 80%
- 문구 안정(최근 수정 없이 재사용)
→ 모두 충족 시에만 사람 확인 후 main 수동 병합 또는 승격
```
...
Prompt 후보 등록(Drive + GitHub prompts branch)
  ...
  → 월간 또는 분기 정리 때 검증
  → owner 지정
  → main 수동 병합 또는 assets/skills/company-* 로 승격
  → installer 배포
```
...
| Reviewer | AI governance / 계정 담당자 | candidate→main 수동 병합 검토, 정리(중복/deprecate), 승격 게이트 판정 |
