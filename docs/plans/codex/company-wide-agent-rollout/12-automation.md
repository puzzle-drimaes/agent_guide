...
1차 도입에서는 **회고/노하우 작성을 자동으로 강제·추적하지 않는다.** 자동화는 Google Drive + GitHub branch 흐름을 쉽게 만드는 쪽만 먼저 한다. 실시간 메신저와 Notion 자동화는 이번 버전에서 제외한다.

```text
- 먼저: agent 기반 Google Drive 피드백 `.md` 업로드 안내, 공유 폴더 업로드 안내, 신규 .md 알림, GitHub prompts/skills branch commit 보조.
...
5. PR 공유 문서 reminder (가벼운 권유, 강제 아님)
6. Weekly reminder (Drive 운영 메모 기준의 가벼운 리마인더, 제출률 추적 없음)
7. Governance dashboard (참고지표 위주)
```

## 0. Feedback upload helper
